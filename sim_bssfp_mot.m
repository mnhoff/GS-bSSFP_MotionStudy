function [tempa,tempb,tempm,theta,cdata,tempE,tempr,noisonly,nonmotdata,motdata,row1,rowlst,col1,collst,freqs,amplitudes,masque] = sim_bssfp_mot(cyc,noisval,th_pts,flip,nul,agran,TR,motdir,pr)
...
freqs = [1 5:5:60]; amplitudes = [0 1 2 3:2:27 30];
% freqs = 1:2:60; amplitudes = 0:30;% 30x31 ampsxfreqs
...
for fr = freqs
    ...
    for amp = amplitudes
        cnta = cnta+1;
        if strcmpi(motdir,'F')% if you ever want to choose FE/PE AND motion directions, mimic simsanmotion2.m call from sim_bssfp_continuous_shft4.m
            [motdata(:,:,cntf,cnta),motKSP] = transMotGen(cdata(:,:,1), TR, 'sinXampl', amp, 'sinXfreq', fr );
        else %default is PE motion
            [motdata(:,:,cntf,cnta),motKSP] = transMotGen(cdata(:,:,1), TR, 'sinYampl', amp, 'sinYfreq', fr );
        end
    end
end
